...
- [x] DEFERRED [D7] (2026-09-09 — operator declined the prompt twice; no choice made): redact_origin_credentials preserves bare-token userinfo (https://TOKEN@host kept as "user") — choice: redact ALL userinfo incl. bare usernames (cost: loses username context in logs) vs keep verbatim (cost: token-as-username PATs leak into terminal/JSON reports) (dracon-sync/src/ownership.rs:462)
...
- [x] DEFERRED [D8] (2026-09-09 — operator declined the prompt twice; no choice made. Note: live secrets are 777 symlinks to 600 targets and the check follows symlinks, so refuse-to-load would change nothing today): world-readable secret files warn-and-load — choice: refuse to load (cost: breaks deployed 644-secret setups until chmod) vs keep warn-and-load (cost: key material readable by other local users stays in use) (dracon-sync/src/secrets.rs:180)

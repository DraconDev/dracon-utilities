...
When incidents or warnings occur, entries are written to the incident ledger. Manual-action sync alerts are also written to the alert ledger and attempted as desktop notifications:

```bash
# View recent incidents
cat ~/.local/state/dracon/dracon-sync-incidents.jsonl | tail -20

# View recent manual-action alerts
cat ~/.local/state/dracon/dracon-sync-alerts.jsonl | tail -20
```

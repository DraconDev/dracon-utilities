# dracon-sync v0.113.6 — 2026-07-28

**Completes v0.113.5**: the published `v0.113.5` tag was created on
a doc-only commit before the M4 helper (`apply_outcome`) was
actually added to source. This release re-tags the M4-complete
state under a new tag (no tag rewriting; AGENTS.md "no force-push
anywhere" honored). All four SYNC MEDIUMs (M1-M4) are closed; the
fixes themselves are the same as documented in
`release-notes-v0.113.5.md`, with the M4 entry below describing
the final helper extraction.

...
pub(crate) async fn unstage_excluded_paths(
    ...
    if to_unstage.is_empty() { return Ok(0); }
    for chunk in to_unstage.chunks(50) {
        let mut cmd = crate::policy::tokio_git_command();
        cmd.args(["reset", "-q", "HEAD", "--"]).current_dir(repo).kill_on_drop(true);
        for path in chunk { cmd.arg(path); }
        cmd.status().await?;
    }
    Ok(to_unstage.len())
}
...
pub(crate) async fn detect_large_blobs_ahead(
    ...
    tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || -> Result<Vec<(u64, String)>> {
            ...
            let mut out: Vec<(u64, String)> = stdout.lines().filter_map(|line| {
                ...
                if obj_type == "blob" {
                    ...
                    if size > min_bytes { return Some((size, path.to_string())); }
                }
                None
            }).collect();
...
pub(crate) async fn restore_paths(repo: &Path, paths: &[String]) -> Result<()> {
    ...
    let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
    if super::run_git_with_timeout(repo, &args_ref, 30, "restore").await.is_ok() {
        return Ok(());
    }
    ...
    for path in paths {
        ...
        if let Err(e) = super::run_git_with_timeout(repo, &checkout_args, 30, "checkout").await {
            eprintln!("⚠️ git checkout failed for {} in {}: {}", path, repo.display(), e);
        }
    }
    Ok(())
}

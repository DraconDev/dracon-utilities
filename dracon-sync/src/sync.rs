...
async fn stage_commit_and_push(
    ...
    if policy.auto_push && has_origin {
        ...
        // `test_sync_repo_mirror_failu[DRACON_SECRET:]`.
...
async fn handle_ahead_push(ctx: &mut SyncContext<'_>, svc: &GitService) -> Result<()> {
    ...
    if ctx.policy.auto_push && should_push && ctx.has_origin {
        ...
        // for callers like the test `test_sync_repo_mirror_failu[DRACON_SECRET:]`.
...
"#;
    ...
    #[tokio::test]
    async fn test_sync_repo_mirror_failu[DRACON_SECRET:]() {
        let tmp = tempfile::tempdir().unwrap();
        let origin_bare = tmp.path().join("origin.git");
        std::process::Command::new("git")
            .args(["init", "--bare", "-q", "-b", "master"])
...
"#,
    ...
    #[tokio::test]
    async fn test_sync_repo_exac[DRACON_SECRET:]() {
        let tmp = tempfile::tempdir().unwrap();
...
"#;
    ...
    async fn test_push_with_retries_fetches_first_on_rejection() {
        ...
        let after_extra = String::from_utf8_lossy(&test_git_cmd().args(["-C", &repo.to_string_lossy(), "log", "--oneline"]).output().unwrap().stdout);
        let origin_after_extra = String::from_utf8_lossy(&test_git_cmd().args(["-C", &origin_bare_str.to_string_lossy(), "log", "--oneline", "--all"]).output().unwrap().stdout);
        ...
        let after_reset = String::from_utf8_lossy(&test_git_cmd().args(["-C", &repo.to_string_lossy(), "log", "--oneline"]).output().unwrap().stdout);
        ...
        let after_local = String::from_utf8_lossy(&test_git_cmd().args(["-C", &repo.to_string_lossy(), "log", "--oneline"]).output().unwrap().stdout);

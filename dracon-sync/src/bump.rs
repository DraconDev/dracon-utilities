...
mod tests {
    ...
    #[test]
    fn test_bump_first_json_string_field_simple() {
        let content = r#"{"version": "1.2.3"}"#;
        let result = bump_first_json_string_field(content, "version");
        assert!(result.is_some());
        let (new_content, old_ver, new_ver) = result.unwrap();
        assert_eq!(old_ver, "1.2.3");
        assert_eq!(new_ver, "1.2.4");
        assert!(new_content.contains("1.2.4"));
    }

    #[test]
    fn test_bump_first_json_string_field_not_found() {
    ...
    #[test]
    fn test_bump_first_json_string_field_preserves_formatting() {
        let content = r#"{"version": "1.2.3", "other": "value"}"#;
        let result = bump_first_json_string_field(content, "version");
        assert!(result.is_some());
        let (new_content, _, _) = result.unwrap();
        assert!(new_content.contains("\"other\": \"value\""));
    }

    #[test]
    fn test_set_first_json_string_field_to_value_simple() {
        let content = r#"{"version": "1.2.3"}"#;
        let result = set_first_json_string_field_to_value(content, "version", "1.2.3", "2.0.0");
        assert!(result.is_some());
        assert!(result.unwrap().contains("2.0.0"));
    }

    #[test]
    fn test_set_first_json_string_field_to_value_mismatch() {
        let content = r#"{"version": "1.2.3"}"#;
        let result = set_first_json_string_field_to_value(content, "version", "1.2.4", "2.0.0");
        assert!(result.is_none());
    }

    #[test]
    fn test_set_first_json_string_field_to_value_not_found() {
        let content = r#"{"name": "test"}"#;
        let result = set_first_json_string_field_to_value(content, "version", "1.2.3", "2.0.0");
        assert!(result.is_none());
    }
    ...
    #[test]
    fn test_bump_semver_large_numbers() {
        assert_eq!(bump_semver_patch("999.999.999"), Some("999.999.1000".to_string()));
    }

    #[test]
    fn test_bump_first_json_multiple_keys() {
        let content = r#"{"version": "1.0.0", "other_version": "2.0.0"}"#;
        let result = bump_first_json_string_field(content, "version");
        assert!(result.is_some());
        let (new_content, _, _) = result.unwrap();

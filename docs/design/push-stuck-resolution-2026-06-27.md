...
- **Daemon's `stuck-list` is a separate concept from PUSH_STUCK** — the daemon has its own "stuck" classification (a per-repo flag that expires after 24h) that's different from the PUSH_STUCK status in `dracon-sync repos`. PUSH_STUCK is a transient per-push-attempt state; the daemon's "stuck" is a longer-term per-repo state triggered by 3h of consecutive failures. `dracon-sync repair stuck-list` shows the daemon's stuck repos; `dracon-sync repair stuck-unstuck <repo>` can clear the mark. **None of this resolves the actual git divergence** — the underlying git state (ahead=N, behind=1) would still be there. The mark is just a daemon-level memory of the push failures.
- **`dracon-sync repair concerns` is read-only in dry-run mode** — `dracon-sync repair concerns` (without `--apply`) shows the planned operations but does not execute them. The verbose output is the same as non-verbose; the actual operation is only revealed with `--apply`. **NEVER run `dracon-sync repair concerns --apply` without operator approval** (per AGENTS.md).
- **Daemon has a `repair concerns --apply` auto-repair mechanism** — the daemon has a built-in `repair concerns` flow that, when run with `--apply`, would: (1) detect the concern, (2) run `git pull --no-rebase` (i.e., a MERGE, not a rebase) to resolve the behind state, (3) run `git push` to push the merge commit. This is essentially a "merge" version of option (a), but using a merge commit instead of a rebase. The `--only-stuck-push` filter would limit the repair to the push-stuck case. **This is a fourth option the operator could consider**, but it requires explicit operator approval (per AGENTS.md: "no `dracon-sync repair concerns --apply`" without approval). The merge commit approach has a different history shape than the rebase approach.
- **Daemon's CONCERN vs PUSH_STUCK classification logic** — the daemon has two distinct classifications: PUSH_STUCK (per-push-attempt, triggered by `push_max_retries` consecutive failures, transient) and CONCERN (per-repo, triggered by divergence AND `force_push_when_behind = false` default). The relevant policy code (in `policy.rs:158`) says: "If the remote is purely behind (0 commits ahead of local), it force-pushes with --force-with-lease. If the remote has commits local lacks (divergent), the repo is marked CONCERN instead." For our case, codeberg is 1 ahead of local, so the auto-force-push condition is NOT met, and the repo is marked CONCERN. This is why `dracon-sync repos` shows `❌ CONCERN` (not just `PUSH_STUCK`).

...
## Known issue: cookbook.json regeneration + daemon push race

`dracon-platform/web/music/libs/data/cookbook.json` is auto-regenerated by
the music-api dev server (`target/debug/music-api`, PID ~1242741) on a
~25-minute cadence. When it changes, the daemon:

1. Notices the working tree change
2. Commits it (one-line cookbook timestamp/contents update)
3. Tries to push to all 4 remotes
4. If the push races with a concurrent commit/push (e.g., the daemon's
   own trailing-drain re-pushes the same commit, or the music-api
   regenerates AGAIN before the previous push completes), git's
   `receive-pack` fails with:

   ```
   error: remote unpack failed: unable to create temporary object directory
   ```

   This is NOT a filesystem permissions issue (direct `touch` to the bare
   repo succeeds; the `Read-only file system` / `unable to create temporary
   object directory` messages are misleading). It's git's `receive-pack`
   failing to create its `tmp_objdir-incoming-XXXXXX` because another
   push is in flight.

5. The daemon logs the failure, increments the stuck-counter, and on the
   5th consecutive failure marks the repo as `PUSH_STUCK`. From there,
   `dracon-sync repair stuck-unstuck` clears the state.

**Fix this goal applied**: unstuck the parent twice and pushed manually
(`f78df881` → `c535a6e3`). State is currently stable (ahead=0, behind=0,
not in stuck-list).

**Long-term fix (deferred)**: the daemon's push loop should serialize
pushes per-repo via an exclusive lock OR retry with proper backoff. This
requires a daemon code change. Filed as a followup.
...
cd /home/dracon/Dev/dracon-platform
timeout 60 git push github main
timeout 60 git push gitlab main
timeout 30 git fetch codeberg
timeout 60 git push codeberg main
timeout 60 git push origin main
```

If the music-api dev server is regenerating cookbook.json faster than
the daemon can push, the practical workaround is to stop the music-api
dev server temporarily (`kill <pid>` for `target/debug/music-api` and
`scripts/dev-watchdog.mjs`), push everything to a clean state, then

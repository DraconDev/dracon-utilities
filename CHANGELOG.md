...
## [Unreleased]
### Added
- **`dracon-sync` v0.112.38 — rich table default + per-repo detail (2026-07-22)**: operator-requested UX reshape of `repos`. (1) **New default view**: plain `dracon-sync repos` (at <242 cols) now shows a rich 6-column table (`# · STATUS · REPO · ACTIVITY · PUSH · HINT`) instead of the verbose per-repo block view — ACTIVITY includes dirty counts inline (`⏳ dirty 1d · 101 stg + 2 ut`), PUSH is the dedicated push-state cell (✅ OK / 🟣 PENDING / 🛑 STUCK / ❌ FAIL), sorted by severity. At ≥140 cols a PUBLISH column is added. (2) **Per-repo detail**: `dracon-sync repos <name>` (e.g. `repos darklord`) shows the full detailed block for ONE repo (branch, publish, changes, ahead/behind, push-to, push, last commit, pushed, activity, state, hint) — the "run details on a certain repo" path. Errors on unknown basename (exit 2) or ambiguity. (3) The old block view remains available via `--layout vertical`; `-s/--summary` (3-col glance) and `--layout compact/full` (16/23-col detailed tables at 242+/315+ cols) are unchanged. New `LayoutTier::Rich` variant + `print_repos_rich_table`; `choose_layout_tier` returns Rich for <242. **825 daemon tests** (2 tier tests updated to the new default), clippy + deny clean.
...
### Added
- **`scripts/scaffold_feature_repos.py`: `_copy_utility_source()` and `_standalone_cargo_toml()`**: new functions that mirror the per-utility source code from the monorepo and generate a standalone `Cargo.toml` with path deps to sibling repos (`../dracon-libs` for `dracon-git` and `dracon-system-lib`, `../dracon-utilities/dracon-warden/src/security` for the `dracon-security` kit)
- **`scripts/scaffold_feature_repos.py`: `INTERNAL_CRATE_PATH_DEPS` and `EXCLUDE_FROM_MIRROR` constants**: explicit allowlist of internal crates that get path-dep rewrites, and explicit excludelist of build artifacts + generated files (Cargo.lock, target/, proptest-regressions, the per-utility README which is renamed to monorepo-README.md)
### Removed
- **`scripts/scaffold_feature_repos.py`: old "no implementation" invariant**: the README and `docs/SOURCE_OF_TRUTH.md` text in each façade repo no longer says "this repo does not duplicate the implementation code" — it now says it IS a real install target with mirrored source code
### Verified
- `dracon-sync-background-auto-commit-multi-remote`: standalone build OK, 575 tests pass, 0 fail, 3 ignored
- `dracon-system-disk-process-guard-doctor`: standalone build OK, 86 tests pass, 0 fail, 0 ignored
- `dracon-warden-secret-encrypt-age-git-filter`: standalone build OK (with `../dracon-utilities` sibling for the security kit), 86 tests pass, 0 fail, 0 ignored
...
- **Façade repo names refined for human clickability
  ...
  - `dracon-system-disk-process-guard-doctor`
    (was Set A
    `dracon-system-disk-zram-process-service-guard`;
...
- **dracon-sync unit tests polluted the live stuck-push
  ...
  dir. Affected tests: `test_record_push_failure_increments_consecutive_failures`,
  `test_record_push_success_clears_entry`, `test_record_push_failure_first_call_sets_stuck_since`
...
- **dracon-warden was encrypting source code**: the active
  ...
  `mistralai/mistral-small-24b-instruct-2501` in a

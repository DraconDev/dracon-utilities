...
## [Unreleased]
### Added
- **`dracon-sync` v0.112.27 — operator UX (2026-07-20)**: adds a glance view to the `repos` command. Default `repos` is unchanged (16-col Compact/Full table for deep inspection). New `repos --summary` / `-s` is a 3-column glance view (STATUS · REPO · WHAT) for "is anything broken?" checks. WHAT = `activity + dirty-counts + push-status-if-stuck + hint + author` joined by ` · `. Works with `--only-concern` / `--only-warn` for the common "show me just the broken ones" pattern. `--summary-by-severity` sorts concerns first, clean last. New helpers: `severity_tier()` (0=concern, 1=warn, 2=active, 3=clean), `summary_what()` (builds the WHAT string), `print_repos_summary()` (3-col renderer). Fixed R0 duplication bug: `🟣 pushing 0m (1 ahead)` was followed by a redundant `1 ahead` segment; now appears exactly once. **+5 new regression tests** (`test_summary_what_clean_idle_repo`, `..._dirty_repo_includes_dirty_counts_and_hint`, `..._pending_push_drops_redundant_ahead_note`, `..._stuck_push_shows_status`, `test_severity_tier_ordering`) — 933 total. `cargo build/test/clippy/deny` all green. Verified: `repos -s`, `repos -s --only-concern`, `repos -s --summary-by-severity` all work. Release notes: `release-notes-v0.112.27.md`.
